...
@update:model-value="(val) => toggleTodo(todo.id, val as boolean)"
...
class="text-sm font-medium truncate-2-lines transition-all"
...
:color="isOverdue(todo) ? 'error' : 'neutral'"
...
v-if="todo.completed && todo.completed_at"
...
<span v-if="todo.scenario_source" class="text-xs text-gray-400 flex items-center gap-1 bg-gray-100 dark:bg-gray-800 px-1.5 rounded border border-gray-200 dark:border-gray-700">
...
<div v-else class="text-center py-6 border-2 border-dashed border-gray-200 dark:border-gray-800 rounded-lg">
...
const todosRef = collection(db, 'evenements', props.eventId, 'todos')

// Tri : Non faites d'abord, puis par date de création

const q = query(todosRef, orderBy('completed', 'asc'), orderBy('created_at', 'desc'))

const { data: todos } = useCollection(q, { ssrKey: 'todos-'+props.eventId })
